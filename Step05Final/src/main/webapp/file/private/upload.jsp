...
<%
	...
	MultipartRequest mr = new MultipartRequest(request,//내부적으로 필요한 HttpServlerTrquest 객체전다
	...
	//따라서 mr에 들어있는 참조값을 이용해서 폼 전송된 값을 추출해서 DB에 저장만 잘 하면 된다
	
	
	
	//2. 폼 전송되는 title 을 읽어온다.
	String title = mr.getParameter("title"); 				//<input type="text"  name="title" id="title"/>
	//3.파일의 작성자(업로더)는 HttpSession 객체에서 읽어온다.
	String writer=(String)session.getAttribute("id");
	//4.추가로 원본파일명, 저장된파일명, 파일사이즈도 얻어내서 FileDto 객체에 담아서
	String orgFileName = mr.getOriginalFileName("myFile");  //<input type="file" name="myFile" id="myFile" />
